...
def main() -> int:
    if len(sys.argv) != 2:
        print("Usage: run_mrd_nuclear_guard.py <input.yaml>")
        return 2
    ...
    report = run_pipeline(
        claim,
        default_judges(strict_trace=False, include_nuclear=True),
    )
    verdict = str(report.get("verdict"))

    out: Dict[str, Any] = {
        "module": "mrd_nuclear_guard",
        "input": str(inp.name),
        "verdict": verdict,
        "report": report,
        "timestamp": time.time(),
    ...
    outpath = outputs / f"nuclear_guard.{inp.stem}.{int(time.time())}.report.json"
