#!/usr/bin/env python3
"""MRD — EFT & Operational Renormalization (Executable)."""
import sys, pathlib
...
from ilsc_mrd.modules import eft_ren
from ilsc_mrd.utils.data_eft_ren import load_eft_datum
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python scripts/run_mrd_eft_ren.py <inputs/mrd_eft_ren/*.yaml>")
        raise SystemExit(2)
    yml = pathlib.Path(sys.argv[1])
    cfg = yaml.safe_load(yml.read_text(encoding="utf-8"))

    ds = cfg.get("dataset") or {}
    dat = load_eft_datum(_resolve(ds["csv_path"]), _resolve(ds["meta_path"]))
    cfg.setdefault("scales", {})
    cfg["scales"]["E_probe_GeV"] = dat.row["E_probe_GeV"]
    cfg["sigma_data"] = dat.row["sigma"]
    ...
    artifact = eft_ren.compile(cfg)
    locks, diag = eft_ren.check(artifact, cfg)
