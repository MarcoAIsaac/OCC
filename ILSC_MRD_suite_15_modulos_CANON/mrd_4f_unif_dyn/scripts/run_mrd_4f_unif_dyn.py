#!/usr/bin/env python3
"""MRD — 4F Dynamic Unification Executable."""
import sys, pathlib
...
from ilsc_mrd.modules import fourf_unif_dyn
from ilsc_mrd.utils.data_unif_dyn import load_couplings
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python scripts/run_mrd_4f_unif_dyn.py <inputs/mrd_4f_unif_dyn/*.yaml>")
        raise SystemExit(2)
    yml = pathlib.Path(sys.argv[1])
    cfg = yaml.safe_load(yml.read_text(encoding="utf-8"))

    ds = cfg.get("dataset") or {}
    dat = load_couplings(_resolve(ds["csv_path"]), _resolve(ds["meta_path"]))
    cfg.setdefault("omegaI", {})
    cfg["omegaI"]["mu0_GeV"] = dat.row["mu0_GeV"]
    cfg["couplings_at_mu0"] = {"g1": dat.row["g1"], "g2": dat.row["g2"], "g3": dat.row["g3"]}

    data_obj = {
        "case": cfg.get("case",{}),
        "omegaI": cfg.get("omegaI",{}),
        "betas": cfg.get("betas",{}),
        "unification": cfg.get("unification",{}),
    ...
    artifact = fourf_unif_dyn.compile(cfg)
    locks, diag = fourf_unif_dyn.check(artifact, cfg)
