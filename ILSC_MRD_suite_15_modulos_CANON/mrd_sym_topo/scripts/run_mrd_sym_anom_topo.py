#!/usr/bin/env python3
"""MRD — Symmetries/Anomalies/Topology (Executable)."""
import sys, pathlib
...
from ilsc_mrd.modules import sym_anom_topo
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python scripts/run_mrd_sym_anom_topo.py <inputs/mrd_sym_topo/*.yaml>")
        raise SystemExit(2)
    yml = pathlib.Path(sys.argv[1])
    cfg = yaml.safe_load(yml.read_text(encoding="utf-8"))

    ds = cfg.get("dataset") or {}
    d = load_sym_topo(_resolve(ds["csv_path"]), _resolve(ds["meta_path"]))
    if not (cfg.get("fermions") or []):
        cfg["fermions"] = d.fermions
    cfg.setdefault("topology", {})
    if "chern_k" not in (cfg.get("topology") or {}):
        cfg["topology"]["chern_k"] = d.chern_k
    ...
    artifact = sym_anom_topo.compile(cfg)
    locks, diag = sym_anom_topo.check(artifact, cfg)
