#!/usr/bin/env python3
"""MRD — Effective Gravity IR / GW / PPN (Executable)."""
import sys, pathlib
...
from ilsc_mrd.modules import grav_ir
from ilsc_mrd.utils.data_grav_ir import load_bounds
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python scripts/run_mrd_grav_ir.py <inputs/mrd_grav_ir/*.yaml>")
        raise SystemExit(2)
    yml = pathlib.Path(sys.argv[1])
    cfg = yaml.safe_load(yml.read_text(encoding="utf-8"))

    ds = cfg.get("dataset") or {}
    b = load_bounds(_resolve(ds["csv_path"]), _resolve(ds["meta_path"]))
    cfg["bounds"] = b.row
    ...
    artifact = grav_ir.compile(cfg)
    locks, diag = grav_ir.check(artifact, cfg)
