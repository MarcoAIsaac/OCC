#!/usr/bin/env python3
"""MRD — Rama efectiva / Decoherencia / Objetividad (Executable)."""
import sys, pathlib
...
from ilsc_mrd.modules import rama_ef
from ilsc_mrd.utils.data_rama_ef import load_thresholds
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python scripts/run_mrd_rama_ef.py <inputs/mrd_rama_ef/*.yaml>")
        raise SystemExit(2)
    yml = pathlib.Path(sys.argv[1])
    cfg = yaml.safe_load(yml.read_text(encoding="utf-8"))

    ds = cfg.get("dataset") or {}
    th = load_thresholds(_resolve(ds["csv_path"]), _resolve(ds["meta_path"]))
    cfg.setdefault("times", {})
    cfg.setdefault("thresholds", {})
    cfg["times"]["tau_read"] = th.row["tau_read"]
    cfg["times"]["tau_stab"] = th.row["tau_stab"]
    cfg["thresholds"]["R_min"] = th.row["R_min"]
    ...
    data_obj = {
        ...
        "pointer_basis": cfg.get("pointer_basis","Z"),
        "dataset_id": th.dataset_id,
        "dataset_hashes": {
    ...
    artifact = rama_ef.compile(cfg)
    locks, diag = rama_ef.check(artifact, cfg)
