...
def _apply_judge_certificates(cfg: Dict[str, Any], locks: Dict[str, Any], diagnostic: list) -> None:
    ...
    Audit-side only: does not replace physics checks.
    Adds a short human-readable line to `diagnostic` and stores details in `locks['CERTS']`.
    """
    jc = cfg.get("judge_certs", {}) or {}
    if not jc:
        locks["CERTS"] = {"verdict": "SKIP", "note": "no judge_certs provided", "pass": True, "severity": "FAIL"}
        diagnostic.append("CERTS: SKIP (no judge_certs)")
        return
    ...
    try:
        if (jc.get("PA", {}) or {}).get("enabled", False):
            cert = load_cert(str(repo_path / jc["PA"]["path"]))
        ...
        if (jc.get("RFS", {}) or {}).get("enabled", False):
            cert = load_cert(str(repo_path / jc["RFS"]["path"]))
        ...
        if (jc.get("IO", {}) or {}).get("enabled", False):
            cert = load_cert(str(repo_path / jc["IO"]["path"]))
            cv = validate_io(cert)
            out["IO"] = {"verdict": cv.verdict, "note": cv.note}
        else:
            out["IO"] = {"verdict": "SKIP"}

        verdict = "PASS" if all(v.get("verdict") in ("PASS", "SKIP") for v in out.values()) else "FAIL"
        out["verdict"] = verdict
        out["pass"] = (out.get("verdict") in ("PASS","SKIP"))
        out["severity"] = "NO-EVAL" if out.get("verdict")=="NO-EVAL" else "FAIL"
    except Exception as e:
        out = {"verdict": "NO-EVAL", "error": str(e), "pass": False, "severity": "NO-EVAL"}

    locks["CERTS"] = out
    diagnostic.append(f"CERTS: {out.get('verdict','NO-EVAL')}")
...
def main():
    ...
    # Also write a flat JSON report for suite-wide tooling consistency
    try:
        from pathlib import Path as _Path
        case_stem = _Path(args.input_yaml).stem
        flat = {
            "mrd": "mrd_sk",
            ...
            "locks": res.get("locks", []),
            "first_reason": res.get("first_reason") or res.get("note") or "",
            "certificates": res.get("certificates", {}),
            "artifacts": {
                "result_json": os.path.join(args.outdir, res["case_id"], "result.json"),
                "report_pdf": os.path.join(args.outdir, res["case_id"], "report.pdf"),
            }
        }
        write_json(os.path.join(repo_root, args.outdir, case_stem + '.report.json'), flat)
    except Exception as e:
        # Do not fail execution if the flat report cannot be produced
        pass
