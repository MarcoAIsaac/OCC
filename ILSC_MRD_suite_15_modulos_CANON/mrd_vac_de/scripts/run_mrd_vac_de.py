#!/usr/bin/env python3
"""MRD — Vacuum / Dark Energy (Executable)."""
import sys, pathlib
...
from ilsc_mrd.modules import vac_de
from ilsc_mrd.utils.data_vac_de import load_vac_bounds
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python scripts/run_mrd_vac_de.py <inputs/mrd_vac_de/*.yaml>")
        raise SystemExit(2)
    yml = pathlib.Path(sys.argv[1])
    cfg = yaml.safe_load(yml.read_text(encoding="utf-8"))

    ds = cfg.get("dataset") or {}
    b = load_vac_bounds(_resolve(ds["csv_path"]), _resolve(ds["meta_path"]))
    cfg["bounds"] = b.row
    ...
    artifact = vac_de.compile(cfg)
    locks, diag = vac_de.check(artifact, cfg)
